...
async fn spawn_server(config: AppConfig) -> Result<(SocketAddr, JoinHandle<Result<()>>)> {
    ...
    let executor_program = Some(std::ffi::OsString::from(env!("CARGO_BIN_EXE_narrowd")));
    let task = tokio::spawn(async move {
        sshd::run_on_listener_unsandboxed_for_tests(config, listener, executor_program).await
    });
